...
function PlayTicTacToe ({players, activeBoard, currentGameActivities, activePlayer, previousGames}) {
  ...
  useEffect(() => {
    if (currentGameActivities.length) {
      if (currentGameActivities[currentGameActivities.length - 1].player.symbol === 'O') {
  ...
  const onChangeBoard = async(row, column) => {
    ...
    if (result.verdict || activities.length === 9) {
      let lastGameDetails = {
        ...
        activities: currentGameActivities,
        finalBoard: board,
  ...
  return (
    <Flex>
      <Flex flex="2" direction="column" border="1px" borderColor={useColorModeValue('green.100', 'green.900')} mr={5} bg={useColorModeValue('green.50', 'green.900')}>
        ...
        <ActivityLog activityList={currentGameActivities} />
      </Flex>
